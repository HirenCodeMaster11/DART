...
int main() {
  for (int i = 0; i < bankData.length; i++) {
    BankModel p1 = BankModel.fromBank(bank: bankData[i]);
    server.add(p1);
  }

  for (int i = 0; i < server.length; i++) {
    print('Is Active : ${server[i].isActive}  ' +
        'Balance : ${server[i].balance}  ' +
        'Age : ${server[i].age}  ' +
        'Eye Color: ${server[i].eyeColor}  ' +
        'Name : ${server[i].name}  ' +
        'Genders: ${server[i].gender}  ' +
        'Company : ${server[i].company}  ' +
        'Email : ${server[i].email}  ' +
        'Phone : ${server[i].phone}  ' +
        'Address : ${server[i].address} ');
  }
  return 0;
}
